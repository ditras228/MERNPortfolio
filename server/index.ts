...
app.post('/works', async (req: express.Request & any, res: express.Response) => {
    const auth = req.headers.authorization
    if (jwt.verify(auth, process.env.JWT_SECRET)) {
        const body = req.body
...
const start = async () => {
    try {
        await mongoose.connect('mongodb+srv://admin:admin@cluster0.ohprq.mongodb.net/myFirstDatabase?retryWrites=true&w=majority')
