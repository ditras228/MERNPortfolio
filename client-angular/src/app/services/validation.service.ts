  ...
  public GetValidationMessage(
    ...
    errors = Object.keys(formControl).reduce((acc, key) => {
      if (!formControl[key].valid) {
        const validationRuleKey = Object.keys(
          form.controls[key].errors || {}
        ).shift();

        switch (validationRuleKey) {
          case 'minlength':
            acc[key] = this.getMessage(validationRuleKey, this.passMinLength);
            break;
          case 'maxlength':
            acc[key] = this.getMessage(validationRuleKey, this.passMaxLength);
            break;
          default:
